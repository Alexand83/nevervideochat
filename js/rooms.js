...
export function switchRoom(roomId) {
  ...
  if (isEventsRoom) {
    ...
    if (state.localStream && String(state.cameraRoom) === roomIdStr) {
      import('./camera.js?v=20260466').then(async ({ insertCameraIntoEventsGrid }) => {
        if (state.activeRoom === roomIdStr) {
          ...
          if (ownCamWin && ownCamWin.isEventsGrid) {
            ...
            const { createCameraWindow } = await import('./camera.js?v=20260466');
  ...
  if (isEventsRoom) {
    ...
    if (room) {
      const requestEventsRoomCams = async (label) => {
        ...
        const { requestPublicCamera } = await import('./camera.js?v=20260466');
