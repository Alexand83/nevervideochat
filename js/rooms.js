    ...
    .on('presence', { event: 'leave' }, async ({ key }) => {
      ...
      state.presenceLeaveTimers[timerKey] = setTimeout(async () => {
        ...
        if (state.cameraWindows[uid]) {
          const { closeCameraWindow } = await import('./camera.js?v=20260309');
...
export function switchRoom(roomId) {
  ...
  if (isEventsRoom) {
    ...
    if (state.localStream && String(state.cameraRoom) === roomIdStr) {
      import('./camera.js?v=20260309').then(async ({ insertCameraIntoEventsGrid }) => {
        if (state.activeRoom === roomIdStr) {
          ...
          if (ownCamWin && ownCamWin.isEventsGrid) {
            ...
            const { createCameraWindow } = await import('./camera.js?v=20260309');
  ...
  if (isEventsRoom) {
    ...
    if (room) {
      ...
      setTimeout(async () => {
        ...
        const { requestPublicCamera } = await import('./camera.js?v=20260309');
