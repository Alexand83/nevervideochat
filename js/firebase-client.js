...
import { addMessage, extractQuote, renderMessage, handleReactionUpdate, updateMessageReactions, CHAT_MESSAGES_WINDOW } from './chat.js?v=20260465';
...
export async function showDisconnectedOverlay(forceShow) {
  ...
  messageUnsubscribes = {};
  try {
    const { resetCameraStateOnDisconnect } = await import('./camera.js?v=20260473');
  ...
  try {
    ...
    state.signalCh
      ...
      .on('broadcast', { event: 'cam-watcher-left' }, ({ payload }) => {
        if (String(payload.to) !== String(state.currentUser?.id)) return;
        import('./camera.js?v=20260473').then(({ handleCamWatcherLeftFromSignal }) =>
        ...
        if (camRoom === state.activeRoom && inActiveRoom) {
          ...
          if (isEventsRoom && !state.cameraWindows[fromId] && !state.incomingPCs?.[fromId]) {
            setTimeout(() => {
              ...
              import('./camera.js?v=20260473').then(({ requestPublicCamera }) => requestPublicCamera(fromId, { skipCooldown: true, forceRetry: true, pendingTtlMs: 5000, silentPendingExpiry: true }));
            }, 500);
          }
        }
      })
      ...
      .on('broadcast', { event: 'user-kicked' }, async ({ payload }) => {
        ...
        const { closeAllCamerasForUser } = await import('./camera.js?v=20260473');
      ...
      .on('broadcast', { event: 'user-muted' }, async ({ payload }) => {
        ...
        const { closeAllCamerasForUser, closeCameraWindow } = await import('./camera.js?v=20260473');
