...
import { handleCamRequest, handleCamAccepted, handleWebRTCSignal, handleCamClosed,
         closeCameraWindow, endCall, setRemoteSenderVideoOff } from './camera.js?v=20260313';
...
export async function showDisconnectedOverlay(forceShow) {
  ...
  try {
    const { resetCameraStateOnDisconnect } = await import('./camera.js?v=20260313');
  ...
  try {
    ...
    state.signalCh
      ...
      .on('broadcast', { event: 'user-kicked' }, async ({ payload }) => {
        ...
        const { closeAllCamerasForUser } = await import('./camera.js?v=20260313');
      ...
      .on('broadcast', { event: 'user-muted' }, async ({ payload }) => {
        ...
        const { closeAllCamerasForUser, closeCameraWindow } = await import('./camera.js?v=20260313');
        if (isCurrentUser) {
          await closeAllCamerasForUser(targetId);
