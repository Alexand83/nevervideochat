...
import { handleCamRequest, handleCamAccepted, handleWebRTCSignal, handleCamClosed,
         closeCameraWindow, endCall } from './camera.js?v=20260432';
  ...
  try {
    ...
    state.signalCh
      ...
      .on('broadcast', { event: 'user-kicked' }, async ({ payload }) => {
        ...
        const { closeAllCamerasForUser } = await import('./camera.js?v=20260432');
      ...
      .on('broadcast', { event: 'user-muted' }, async ({ payload }) => {
        ...
        const { closeAllCamerasForUser, closeCameraWindow } = await import('./camera.js?v=20260432');
        if (isCurrentUser) {
          await closeAllCamerasForUser(targetId);
